...
|---|---|---|
| 2026-08-29 | Page actions dissolve into the rail (second pass, same founder session as the rail decision below): the Page panel is description-only; right-click a rail row for a dark-card menu (Duplicate / Export page PNG / Export deck PDF / Delete page — items render only where wired), Delete/Backspace on a FOCUSED row removes the page (scoped to the row's keydown and stopped there so the canvas's element-delete never double-fires; last-page delete refuses), "+" in the rail header remains the blank-page add. Rows also went preview-only earlier the same day — number + real mini, the title lives on as tooltip/aria. | Founder: "the page controls seem pretty unnecessary, lets remove them… replace delete page with just clicking the slide and clicking the delete keyboard… allow duplicating by right clicking the slide preview." The panel buttons were indirection for actions that belong on the thing itself; the menu grammar matches the canvas element menu so the two right-clicks feel like one product. |
| 2026-08-29 | The slide rail is the deck's map AND its reorder control: rows carry the REAL page in miniature (per-scene thumbnails from the export capture, `?scene=` on the thumbnail route; the glyph mini remains only as the never-rendered fallback), and dragging a row IS the reorder — an accent insertion line tracks the drop slot, under a 5px travel threshold a press stays a click. The Move left/right buttons leave the Page panel (Duplicate / + Blank / Delete stay). Freshness without a stampede: each edit re-requests only the edited page's mini (per-scene revs client-side, mtime check server-side); structural ops refresh them all. | Founder, gslides screenshots 2026-08-29: "move right and move left should not be page controls, i should be able to reorder the deck by simply draging the slides… each slide on the left should show a preview of the slide." Direct manipulation over indirection; the rail already IS the list being reordered. Trade-off accepted: keyboard-only reorder goes away with the buttons — dragging is the founder-chosen contract. |
| 2026-08-14 | Generation ceremonies become a STAGE: the waiting panel (steps, honest bar, measured clock, exits) moves to a left rail and the real artifact fills the editor area at full scale — the build shows the latest landed page as a full canvas (follow-newest until the user picks; landed-page chips; every iframe mounts once), the outline writes itself full-scale in display type. The blank slide behind may be covered; a soft scrim quiets what peeks through. The pre-fill wait gets its own honest step, "Laying the foundation — colors, type, chrome", mapped to design:scaffold marks (measured 48.7s on a 3-page build; it was hiding inside "Reading the approved outline" and read as a stall). | Founder, live testing 2026-08-14: "this waiting panel should actually be on the left while everything gets generated on the editor like on full scale, not the little boxes" + "1:20 just reading the outline? too much." Supersedes the same-day thumbnail-grid and grown-card decisions. Companion honesty fixes shipped with it: progressive mid-build pages run injectLogoSrc (landed pages no longer error "LOGO_SRC is not defined" until final assembly), and the transports stop retrying wedged requests (two consecutive timeouts end the ladder — a hung Fireworks hour turned 5×120s×re-emissions into the founder's 35-minute build). |
| 2026-08-14 | "Match style" toggle on Image/Icon marquee generation (default ON, hidden for Element): a new generation resembles the NEWEST generated piece of its kind, server-resolved doc-wide — same model + same seed + a style descriptor of the reference's actual pixels (one Kimi vision read via the sole vision transport, cached on the provenance entry; a descriptor is asset-scoped and is shed when the pixels change). No reference → a fresh generation, silently. | Founder 2026-08-14: "users keep using the same icon family… a toggle to ressemble other icons." Without it every icon is a lone stranger and decks collect a style zoo. Verified live: rocket then matched shield came out unmistakable siblings (sticker rim, bold outlines, shared black/white/orange) with seed+model shared and the descriptor cached after one read. Default ON because family is the normal wish and the no-reference case costs nothing. |
...
| 2026-09-03 | Decks MOVE (founder: "for launch let's allow motion… definitely a differentiator"). Every page opens with an entrance choreography authored by the model at build time (pack MOTION block: reading-order stagger, 500-900ms, opacity + small transform, settled within 1.5s; one restrained ambient loop per page on the graphic device's inner parts only, never text); the element panel gains **Animate** with a prompt beside Regenerate, its own provenance field, and "▶ Replay page". Motion plays on a page's first visit in the editor, in presenting mode, and on shared links; post-edit reloads and morphs land settled so editing stays static and instant; reduced-motion viewers get pages at rest. The one hard invariant — the static style IS the designed end state (`animation-fill-mode: backwards`) — is what keeps export, thumbnails, layout gates and the editor's drag truthful (docs/HARNESS.md § Motion). | Static decks were the launch position for safety; the founder judged motion the level-up that separates the product, and the settle machinery (2026-07 editor, export, measure) already existed, so motion costs zero model budget and no new gates — only a contract the author already excels at. The chrome stays quiet: motion lives in the user's work, never in the app UI around it. |
