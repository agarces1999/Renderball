/**
 * ACCEPTANCE V5 — THE CASTING DECISION. Same Duolingo blueprints as v2, cast
 ...
 *   .data/acceptance5/<model>/scene{0..4}.png.
 *
 * GALLERY (.data/acceptance5/index.html): FOUR rows — zai-glm-4.7,
 ...
 * runs.
 *
 *   set -a && source .env.local && set +a && node scripts/acceptance5-spike.mjs
 *
 * Footprint: this file + scripts/acceptance5-spike.mjs + .data/acceptance5/ +
 * src/generated/CAST_SPIKE_A5_*. Reads everything, modifies nothing else.
 */
...
const OUT_DIR = path.join(ROOT, ".data", "acceptance5");
...
interface RowSpec {
  model: string;
  /** Output subdir name (.data/acceptance5/<dir>/sceneN.png). */
  dir: string;
  genDir: string;
...
const ROWS: RowSpec[] = [
  {
    model: "GLM-5.2 on Fireworks (full cast, thinking disabled)",
    dir: "glm52-fireworks",
    genDir: path.join(ROOT, "src", "generated", "CAST_SPIKE_A5_GLM52"),
...
const runRow = async (
  ...
  if (spec.effortPolicy === "passthrough") {
    ...
    process.env.RB_CAST_MODEL_HERO = "accounts/fireworks/models/glm-5p2";
    process.env.RB_CAST_MODEL_LEAVES = "accounts/fireworks/models/glm-5p2";
  } else {
    process.env.RB_CAST_MODEL = spec.model;
  }
  console.log(`\n════════ ROW ${spec.dir} — ${spec.effortPolicy === "passthrough" ? "HERO+LEAVES=glm-5p2@Fireworks (thinking disabled — the reference author at speed)" : `RB_CAST_MODEL=${spec.model}, effort policy: ${spec.effortPolicy === "none" ? `reasoning_effort "none" (off switch)` : "effort OMITTED"}`} ════════`);

  try {
    // Canary: proves transport + key + THIS model id before castBuild spends.
    await phase(`${spec.dir}:canary`, async () => {
      const c = await budgetedCast(ctx, "canary", { system: "You are a connectivity canary. Reply with exactly: OK", user: "Reply with exactly: OK", maxTokens: 256, model: "accounts/fireworks/models/glm-5p2", effort: "none" });
    ...
    // Final renders → .data/acceptance5/<model>/scene{0..4}.png.
...
const galleryHtml = (g: {
  ...
  <div class="scroll"><table>
    <tr>
      ...
      <th colspan="4">GLM-5.2 @ FIREWORKS (this run)</th>
...
const main = async (): Promise<void> => {
  if (!castConfigured()) {
    console.error("RB_CAST_KEY missing — run: set -a && source .env.local && set +a && node scripts/acceptance5-spike.mjs");
    process.exitCode = 1;
    return;
  }
  ...
  const report: Record<string, unknown> = {
    experiment:
      "ACCEPTANCE V5: the casting decision — v2's Duolingo blueprints/DS/script REUSED VERBATIM (zero head calls); cast executed by zai-glm-4.7 (effort none) and gemma-4-31b (effort omitted) on Cerebras, sequentially; full v2 gate loop per row; 4-row gallery vs gpt-oss (v2) and the GLM-5.2 reference",
    brand: BRAND,
    ref: REF_BUILD,
