/**
 * ACCEPTANCE V7 — the Fireworks GLM 5.2 FAST-tier adoption run + the audited
 * efficiency/quality fixes, verified against v6's measured 676.4s Klarna
 * build. Same brand, same brief, same reference (01KWTTHKKECT0GGZ6D7HBQP1R5);
 * everything head-side GENERATED again. Projection under test: ~3-4 min wall.
 *
 * CHANGES vs v6 (the ACCEPTANCE V7 batch, audit: speed-quality-pivot memory):
 *   A. FAST ROUTING — script + DS + ALL cast calls (hero/leaves/regens/
 *      repairs) run on accounts/fireworks/routers/glm-5p2-fast (A/B measured
 *      232-274 tok/s vs standard's 45-60 that evening; $2.10/$6.60 per M).
 *      Blueprints stay gpt-oss-120b @ Cerebras. The canary phase canaries the
 *      fast router too. HEALTH GUARD: a fast-router ERROR falls back per-call
 *      to the standard tier; sustained <150 tok/s (median of sizeable calls)
 *      degrades ALL subsequent fast-routed calls to standard — loudly, never
 *      dying.
 *   B. POISONED-CACHE FIX (quality-critical) — v6's emission cache replayed
 *      s1/s2.atmosphere's TRUNCATED (stop=length) bodies across all 3 gate
 *      rounds and even into cast-build's repair calls, shipping placeholders.
 *      v7 keys the cache on emission sanity + fragment-syntax success, treats
 *      stop=length as a FAILURE (one retry at a raised cap, never cached),
 *      busts a cached entry when a repair call proves it failed the full
 *      gate, and repair calls ALWAYS hit the model fresh.
 *   C. TOKEN CAPS — atmosphere 2000→3500 and composition-head 16000→28000
 *      landed in PRODUCTION (lib/agents/cast-build.ts MAX_TOKENS_BY_SLOT,
 *      lib/agents/composition-head.ts HEAD_MAX_TOKENS); hero stays 6000.
 *      v6 truncated at exactly both old caps.
 *   D. SCRIPT STAGE (v6: 320.7s) — json:true on script castCalls
 *      (response_format json_object), a salvage parser (first '{',
 *      brace-balanced span) before rejecting, SURGICAL field-patch repairs
 *      for field-scoped validation failures (headline readability, beat
 *      coverage, atmosphere copy-paste, ungrounded claims, type-only scenes)
 *      merged locally instead of full re-emissions, and effort "low"
 *      (bounded 1024 thinking) on FAST — the probe showed thinking-high buys
 *      no measurable voice gain. Full re-emission only when surgical fails
 *      twice or the error isn't field-scoped; ≤3 full attempts as before.
 *   E. OVERLAP — design-system ∥ composition-head via Promise.all the moment
 *      the script validates (audit: zero data dependencies; v6 serialized
 *      them for 21.3s of avoidable wall).
 *   F. SEQUENCE VISION — ONE retry on a malformed/no-JSON verdict (v6
 *      discarded 46.6s of work on a bad parse).
 *   G. QUALITY (production, tested) — checkSceneComposition cross-check: no
 *      interior[] item may retype copy another element ownsCopy (v6's dup
 *      "Get the app" CTA); font-binding normalizer now normalizes NON-primary
 *      families in stacks (v6 scene-3 rendered fallback serif).
 *   H. TELEMETRY — cost attribution keyed by the SERVED model id (v5 bucketed
 *      Fireworks spend under 'cerebras'), fast router reported separately,
 *      observed tok/s per call in the call log.
 *
 * GALLERY (.data/acceptance7/index.html): v7 row vs the v6 row vs the Klarna
 * GLM reference row (v6 + reference frames copied from .data/acceptance6 —
 * same artifacts, no re-render), a v6-vs-v7 phase-timeline comparison table
 * (v6 baseline: script 320.7 · DS 32.8 · comp 21.3 · cast+gates 244.0 ·
 * seq-vision 46.6 · total 676.4), fast-router tok/s, stop-length/cache-bust
 * events, the script attempt/surgical log, and the per-model cost split.
 *
 ...
 *
 * Footprint: this file + scripts/acceptance7-spike.mjs + .data/acceptance7/ +
 * src/generated/CAST_SPIKE_A7_KLARNA. Reads everything, modifies nothing else.
 */
...
const OUT_DIR = path.join(ROOT, ".data", "acceptance7");
/** The v6 run's artifacts — v6 + reference rows are COPIED, never re-rendered. */
const V6_DIR = path.join(ROOT, ".data", "acceptance6");
const GEN_DIR = path.join(ROOT, "src", "generated", "CAST_SPIKE_A7_KLARNA");
...
/** Script ATTEMPT 1 runs effort "low" (bounded 1024 thinking) on FAST —
 *  change D4: the probe showed thinking-high buys no measurable voice gain
 *  and Fast makes retries cheap. Repair RE-EMISSIONS escalate low→medium→high
 *  (run-2 evidence: at flat-low the atmosphere-phrase richness check is a
 *  coin flip — attempts 1-3 ALL emitted a shared cross-scene motion phrase
 *  and the stage went terminal; v6's effort-high attempt 3 cleared the same
 *  class). Surgical patches stay low — they are small, field-scoped edits. */
...
const budgetedCast = async (
  ...
  const fire = async (model: string): Promise<CastResult> => {
    ensureBudget(label);
    totalLlmCalls += 1;
    const r = await castCall({ ...args, model });
    const pm = (perModel[model] ??= { calls: 0, in: 0, out: 0 });
    pm.calls += 1;
    pm.in += r.inputTokens;
    pm.out += r.outputTokens;
    callLog.push({
      label: model === args.model ? label : `${label}:std-fallback`,
      model,
      secs: r.seconds,
      in: r.inputTokens,
      out: r.outputTokens,
  ...
  try {
    return await fire(wantFast && fastRouterDegraded ? FIREWORKS_GLM : args.model);
  } catch (e) {
    // A fast-router ERROR falls back per-call to the standard tier (change A)
    // — loudly, and only for the fast router; every other model's failure is
    // the caller's to handle as before.
    if (!wantFast || fastRouterDegraded || e instanceof BudgetExceeded) throw e;
    const reason = e instanceof Error ? e.message.split("\n")[0] : String(e);
    console.error(`  ▲▲▲ fast router ERRORED on "${label}" (${reason}) — per-call fallback to the STANDARD tier`);
    fastFallbacks.push({ label, reason });
    return fire(FIREWORKS_GLM);
  }
};
...
// ── caching + feedback caller for castBuild (v5's retry machinery + the v7
//    POISONED-CACHE FIX, change B) ─────────────────────────────────────────
...
/** stop=length retries + cache-population refusals — the report's evidence
 *  that change B actually fired (or didn't need to). */
 ...
 *  broken emission must NEVER populate its entry (v6's poison). */
...
const cachingCaller: typeof castCall = async (call) => {
  ...
  // v7 FIX (change B): a cast-build REPAIR call means the body it just ran
  // through the FULL element gate (post-passes + unowned-copy + fragment
  // compile) FAILED. v6 replayed the same cached bytes into the repair —
  // guaranteeing the same failure and shipping placeholders (s1/s2
  // .atmosphere, all 3 rounds). Repairs always hit the model fresh, and the
  // disproven entry is busted so later rounds re-emit too.
  ...
  try {
    ...
    // (RB_CAST_MODEL_HERO/LEAVES are both the FAST router this run).
    const label = `${castRoundLabel}:${pieceId}${feedback ? ":regen" : ""}${isRepair ? ":repair" : ""}`;
    let res = await budgetedCast(label, {
      system: call.system,
      user,
    ...
    // v7 FIX (change B): stop=length is a FAILURE — never accept it, never
    // cache it. ONE retry at a raised cap; still-truncated output returns to
    // cast-build as-is (its gate rejects it and repairs fresh).
    if (res.stopReason === "length") {
      const retryCap = Math.min(Math.ceil(call.maxTokens * 1.8), 12000);
      console.warn(`  [stop=length] ${pieceId}: truncated at cap ${call.maxTokens} — ONE retry at ${retryCap}`);
      const retry = await budgetedCast(`${label}:len-retry`, {
        system: call.system,
        user,
        maxTokens: retryCap,
        model: call.model ?? FIREWORKS_GLM_FAST,
      ...
      stopLengthEvents.push({ label: `${label}`, cap: call.maxTokens, retryCap, recovered: retry.stopReason !== "length" });
      res = retry;
    }
    // v7 FIX (change B): the cache keys on emission sanity + fragment-syntax
    // SUCCESS. v6 cached anything ≥8 chars — including the truncated bodies
    // it then replayed for the rest of the build.
 ...
 *  (v6's processScriptAttempt minus the JSON parse — the salvage parser and
...
const SURGICAL_SYSTEM = [
  ...
  `Rules: patch ONLY the scenes and fields the errors name; leave every other word of the script untouched; NEVER invent numeric claims — replace an ungrounded number with qualitative copy instead; a corrected visual_concept keeps its timed beats ("at 2.4s", "from 3.1s") reaching late into the scene and stays concrete (named drawable elements, real values, furnished containers).`,
].join("\n");
...
// ── sequence vision — ONCE, after the final per-scene round (+ the v7
//    malformed-verdict retry, change F) ─────────────────────────────────────
...
const runSequenceRound = async (
  ...
  // v7 (change F): a malformed/no-JSON verdict earns ONE retry — v6 paid for
...
// ── contrast-cell shape (v7 heroes measured live; reference cells come from
//    the v6 report — its lego pieces are content-named, proxy-sampled there) ──
...
const main = async (): Promise<void> => {
  if (!castConfigured()) {
    console.error("RB_CAST_KEY missing — run: set -a && source .env.local && set +a && node scripts/acceptance7-spike.mjs");
    process.exitCode = 1;
    return;
  }
  ...
  const report: Record<string, unknown> = {
    experiment:
      "ACCEPTANCE V7: the Fireworks GLM-5.2 FAST-tier adoption run on Klarna — script (json+salvage+surgical repairs, effort low) + DS + full cast on glm-5p2-fast; blueprints on gpt-oss-120b @ Cerebras (head cap 28k); poisoned-cache fix (stop=length = failure, cache keys on verified emissions, repairs hit fresh); DS ∥ composition-head; sequence-vision retry; per-model cost attribution. Verifies the ~3-4 min projection vs v6's measured 676.4s.",
    brand: BRAND,
    ref: REF_BUILD,
    v6Baseline: { scriptS: 320.7, designSystemS: 32.8, compositionHeadS: 21.3, castAndGatesS: 244.0, sequenceVisionS: 46.6, totalS: 676.4, totalUsd: 0.7025 },
  ...
  try {
      ...
      for (let attempt = 1; attempt <= SCRIPT_MAX_ATTEMPTS; attempt++) {
        ...
        while (!res.ok && res.fieldScoped && surgicalUsed < SCRIPT_MAX_SURGICAL) {
          ...
          const sr = await budgetedCast(`script-surgical-${surgicalUsed}`, {
            ...
            user: [`THE SCRIPT (JSON):`, JSON.stringify(base), ``, `VALIDATION ERRORS (verbatim):`, res.error, ``, `Emit ONLY the patch object.`].join("\n"),
            maxTokens: 4000,
            model: FIREWORKS_GLM_FAST,
    ...
    //    moment the script validates and are awaited together (v6 serialized
    ...
    //    (lib/agents/composition-head.ts — change C; v6 truncated attempts
    ...
    // ── PHASES 4–5: cast (GLM-5.2-FAST @ Fireworks) + the v6 gate loop ───────
    const castInput = {
      script: script as unknown as Script,
      theme,
    ...
    if (finalCode) await fs.writeFile(path.join(OUT_DIR, "Composition.acceptance7.tsx"), finalCode, "utf8");
    ...
    // ── v7 final renders → .data/acceptance7/v7/scene{0..4}.png ─────────────
    const finalRound = gateRounds[gateRounds.length - 1];
    const v7Cells: RowCell[] = [];
    for (const i of SCENES) {
      ...
      if (exists && !m?.error) {
        await fs.copyFile(shot, path.join(OUT_DIR, "v7", `scene${i}.png`));
        v7Cells.push({ scene: i, png: `v7/scene${i}.png`, label: script.scenes[i]?.label ?? `scene ${i}`, note });
      } else {
        v7Cells.push({ scene: i, label: script.scenes[i]?.label ?? `scene ${i}`, note: `${m?.error ?? "no screenshot"}\n${note}` });
      }
    }

    // ── COMPARISON ROWS: v6 + reference COPIED from .data/acceptance6 ────────
    // Same brand, same reference build — the v6 run's frames and measured
    ...
    const comparison = await phase("comparison-rows", async () => {
      ...
      let v6Report: V6Report | null = null;
      try {
        v6Report = JSON.parse(await fs.readFile(path.join(V6_DIR, "build-report.json"), "utf8")) as V6Report;
      } catch (e) {
        console.warn(`  v6 build-report unreadable (${e instanceof Error ? e.message : e}) — comparison rows will be sparse`);
      }
      const copyRow = async (sub: string): Promise<Map<number, string>> => {
        const out = new Map<number, string>();
        for (const i of SCENES) {
          const ok = await fs
            .copyFile(path.join(V6_DIR, sub, `scene${i}.png`), path.join(OUT_DIR, sub, `scene${i}.png`))
            .then(() => true)
            .catch(() => false);
      ...
      const v6Labels = v6Report?.script?.labels ?? [];
      const v6RowCells: RowCell[] = SCENES.map((i) => ({
        scene: i,
        ...(v6Pngs.has(i) ? { png: v6Pngs.get(i)! } : {}),
        label: v6Labels[i] ?? `scene ${i}`,
        note: noteFor(v6Report?.densityProfile, i),
      }));
      const refCells: RowCell[] = SCENES.map((i) => ({
        scene: i,
        ...(refPngs.has(i) ? { png: refPngs.get(i)! } : {}),
        label: refScript.scenes[i]?.label ?? `scene ${i}`,
        note: noteFor(v6Report?.reference?.densityProfile, i),
      }));
      console.log(`  comparison rows copied: v6 ${v6Pngs.size}/5 · reference ${refPngs.size}/5 (from ${V6_DIR})`);
      return {
        v6Cells: v6RowCells,
        cells: refCells,
        profile: v6Report?.reference?.densityProfile ?? EMPTY_PROFILE,
        contrast: v6Report?.reference?.contrast ?? [],
        v6Profile: v6Report?.densityProfile ?? EMPTY_PROFILE,
        v6Wall: v6Report?.wall?.totalRunSeconds ?? 676.4,
        v6Usd: v6Report?.cost?.totalUsd ?? 0.7025,
      };
    });
    const reference = comparison; // .cells/.profile/.contrast — the gallery's reference row
    report.reference = { densityProfile: reference.profile, contrast: reference.contrast, copiedFrom: V6_DIR };

    // v7 contrast cells for the gallery table (heroes only — cast builds
    // always carry .hero pieces).
    const v7ContrastCells: ContrastCell[] = (finalContrast?.stats ?? []).map((s) => ({
      ...s,
      proxy: false,
    ...
    await phase("gallery", async () => {
      ...
      // v6 → v7 phase comparison (the batch's headline table). v6 baseline
      // from the measured acceptance6 run; v7 aggregates from this run's marks.
      ...
      const v6b = report.v6Baseline as { scriptS: number; designSystemS: number; compositionHeadS: number; castAndGatesS: number; sequenceVisionS: number; totalS: number };
      const cmpRows: [string, number, number][] = [
        ["script", v6b.scriptS, durOf("script")],
        ["design-system", v6b.designSystemS, durOf("design-system")],
        ["composition-head", v6b.compositionHeadS, durOf("composition-head")],
        [`DS ∥ comp-head wall (E)`, Math.round((v6b.designSystemS + v6b.compositionHeadS) * 10) / 10, headWall],
        ["cast + gates", v6b.castAndGatesS, castGatesWall],
        ["sequence-vision", v6b.sequenceVisionS, durOf("sequence-vision-final")],
        ["TOTAL run", v6b.totalS, Math.round(wall.totalRunSeconds * 10) / 10],
      ];
      report.phaseComparison = cmpRows.map(([phaseName, v6s, v7s]) => ({ phase: phaseName, v6s, v7s, deltaS: Math.round((v7s - v6s) * 10) / 10 }));
      const cmpTable = `
  <div class="scroll"><table>
    <tr><th>phase</th><th>v6 (measured, s)</th><th>v7 (this run, s)</th><th>Δ</th><th>×</th></tr>
    ${cmpRows.map(([phaseName, v6s, v7s]) => `<tr><td style="text-align:left">${esc(phaseName)}</td><td>${v6s.toFixed(1)}</td><td class="${v7s <= v6s ? "ok" : "bad"}">${v7s.toFixed(1)}</td><td>${(v7s - v6s).toFixed(1)}</td><td>${v6s > 0 ? (v6s / Math.max(v7s, 0.1)).toFixed(1) : "—"}x</td></tr>`).join("\n")}
  </table></div>`;
  ...
  <div class="scroll"><table>
    <tr>
      ...
      <th colspan="5">v7 — GLM-5.2-FAST @ Fireworks (this run, all heads generated)</th>
      <th colspan="5">Klarna GLM reference (stats from the v6 run's re-measure)</th>
    </tr>
    <tr>
    ...
    <tr>
      ...
      const contrastTable = `
  <div class="scroll"><table>
    <tr><th rowspan="2">scene</th><th colspan="2">v7 hero region</th><th colspan="2">reference (hero or main-diegetic proxy, v6-run stats)</th></tr>
    ...
    ${SCENES.map((i) => `<tr><td>scene ${i}</td>${contrastCellHtml(v7ContrastCells, i)}${contrastCellHtml(reference.contrast, i)}</tr>`).join("\n")}
  </table></div>`;
      ...
      const headline = [
        ...
        `${wall.totalRunSeconds.toFixed(0)}s vs v6's 676s`,
      ].join(" · ");
...
<title>Acceptance v7 — ${esc(BRAND)}: the FAST-tier build vs v6 vs the GLM reference</title>
<style>
...
<body>
  <h1>Acceptance v7 — ${esc(BRAND)}: the FAST-tier build vs v6 vs the GLM reference</h1>
  <div class="banner"><b>THE V7 BATCH:</b> script + DS + full cast on <b>glm-5p2-fast</b> (blueprints stay gpt-oss @
  Cerebras, head cap 28k); the v6 POISONED-CACHE bug fixed (stop=length = failure, cache keys on verified
  emissions, repairs hit the model fresh); atmosphere cap 3500; script stage on json mode + salvage parser +
  SURGICAL field-patch repairs at effort low; DS ∥ composition-head; sequence-vision retry; per-model cost
  attribution. Same Klarna brand/brief/reference as v6 — everything head-side GENERATED again. Projection under
  test: ~3-4 min wall vs v6's measured 676.4s.</div>
  ...
  <h2>Phase timeline — v6 (measured baseline) vs v7 (this run)</h2>
  ${cmpTable}

  <h2>Fast-router health (change A)</h2>
  ${fastHtml}

  <h2>Cache + truncation events (changes B/F)</h2>
  ${cacheHtml}

  <h2>Row — v7 (this run, GLM-5.2-FAST cast)</h2>
  <div class="row">
${rowGrid(v7Cells)}
  </div>

  <h2>Row — v6 (copied from .data/acceptance6 — the 676s standard-tier run)</h2>
  <div class="row">
${rowGrid(comparison.v6Cells)}
  </div>

  <h2>Row — ${esc(BRAND)} GLM reference (frames + stats from the v6 run's re-measure)</h2>
  <div class="row">
${rowGrid(reference.cells)}
  </div>

  <h2>Density — per scene per clause, v7 vs reference (incl. the per-hero floor)</h2>
  ${densityTable}

  <h2>Hero contrast — luminance spread / std-dev per scene, v7 vs reference</h2>
  ${contrastTable}
  ...
  <div class="foot">
    v7 = the FULL v6 pipeline on the FAST tier with the audited fixes: generated script (json mode + salvage
    parser + ≤${SCRIPT_MAX_SURGICAL} surgical field-patch repairs, ≤${SCRIPT_MAX_ATTEMPTS} full attempts, effort ${SCRIPT_EFFORT}, reference fallback only on
    terminal failure) → design system ∥ composition blueprints (Promise.all — change E; head cap 28k) →
    GLM-5.2-FAST element cast (atmosphere cap 3500; stop=length never accepted or cached; repairs hit the model
    fresh) → the v6 gate loop (per-hero density floor + hero-washout, both BLOCKING) → sequence vision ONCE with
    one malformed-verdict retry. v6 + reference rows are the acceptance6 artifacts, copied not re-rendered.
    Budget: ≤${TOTAL_LLM_CEILING} TOTAL LLM calls (text + vision); retries ≤${MAX_RETRY_ROUNDS} rounds.
...
`;
    ...
    console.log("\nV6 → V7 COMPARISON:");
    for (const row of (report.phaseComparison as { phase: string; v6s: number; v7s: number; deltaS: number }[]) ?? []) {
      console.log(`  ${row.phase.padEnd(26)} v6 ${String(row.v6s).padStart(7)}s → v7 ${String(row.v7s).padStart(7)}s  (Δ ${row.deltaS}s)`);
    }
  } catch (err) {
