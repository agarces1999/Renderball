// Tiny runner for scripts/acceptance2-spike.ts — esbuild-bundles the TS and
// imports it (same pattern as scripts/acceptance-spike.mjs). Usage, from the
// repo root:
...
const result = await esbuild.build({
  entryPoints: [join(here, "acceptance2-spike.ts")],
  bundle: true,
  platform: "node",
  format: "esm",
  target: "node18",
...
const work = join(process.cwd(), "node_modules", ".cache", "rb-acceptance2-spike");
mkdirSync(work, { recursive: true });
const outFile = join(work, "acceptance2-spike.bundle.mjs");
