/**
 * ACCEPTANCE V3 — THE CASTING DECISION. Same Duolingo blueprints as v2, cast
 ...
 *   .data/acceptance3/<model>/scene{0..4}.png.
 *
 * GALLERY (.data/acceptance3/index.html): FOUR rows — zai-glm-4.7,
 ...
 *
 * Footprint: this file + scripts/acceptance3-spike.mjs + .data/acceptance3/ +
 * src/generated/CAST_SPIKE_A3_*. Reads everything, modifies nothing else.
 */
import { promises as fs } from "fs";
import path from "path";
import { castBuild, type CastBuildResult } from "../lib/agents/cast-build";
import { castCall, castConfigured, CastProviderError, type CastResult } from "../lib/llm/cast-provider";
...
const OUT_DIR = path.join(ROOT, ".data", "acceptance3");
...
interface RowSpec {
  model: string;
  /** Output subdir name (.data/acceptance3/<dir>/sceneN.png). */
  dir: string;
  genDir: string;
  effortPolicy: "none" | "omit";
  usdPerMIn: number;
  usdPerMOut: number;
}
const ROWS: RowSpec[] = [
  {
    model: "zai-glm-4.7",
    dir: "zai-glm-4.7",
    genDir: path.join(ROOT, "src", "generated", "CAST_SPIKE_A3_ZAI_GLM_4_7"),
    effortPolicy: "none",
    usdPerMIn: 2.25,
    usdPerMOut: 2.75,
  },
  {
    model: "gemma-4-31b",
    dir: "gemma-4-31b",
    genDir: path.join(ROOT, "src", "generated", "CAST_SPIKE_A3_GEMMA_4_31B"),
    effortPolicy: "omit",
    usdPerMIn: 0.99,
    usdPerMOut: 1.49,
  },
...
const budgetedCast = async (
  ...
  const fire = async (effortSent: "none" | "omitted", fireLabel: string): Promise<CastResult> => {
    ...
    ctx.callLog.push({ label: fireLabel, secs: r.seconds, in: r.inputTokens, out: r.outputTokens, stop: r.stopReason, cached: false, effortSent });
    return r;
  };
...
const runRow = async (
  ...
  // The casting model is selected via env — cast-provider reads it per call.
  process.env.RB_CAST_MODEL = spec.model;
  console.log(`\n════════ ROW ${spec.dir} — RB_CAST_MODEL=${spec.model}, effort policy: ${spec.effortPolicy === "none" ? `reasoning_effort "none" (off switch)` : "effort OMITTED"} ════════`);

  try {
    // Canary: proves transport + key + THIS model id before castBuild spends.
    await phase(`${spec.dir}:canary`, async () => {
      const c = await budgetedCast(ctx, "canary", { system: "You are a connectivity canary. Reply with exactly: OK", user: "Reply with exactly: OK", maxTokens: 256 });
    ...
    // Final renders → .data/acceptance3/<model>/scene{0..4}.png.
  ...
  const cerebrasUsd = (ctx.cerebrasIn * spec.usdPerMIn + ctx.cerebrasOut * spec.usdPerMOut) / 1e6;
  ...
  return {
    ...
    finalize,
    cost: {
      cerebras: { calls: ctx.realCalls, inputTokens: ctx.cerebrasIn, outputTokens: ctx.cerebrasOut, usdPerMIn: spec.usdPerMIn, usdPerMOut: spec.usdPerMOut, usd: cerebrasUsd },
...
const galleryHtml = (g: {
  ...
  <div class="scroll"><table>
    <tr>
      ...
      <th colspan="4">A — zai-glm-4.7 (this run)</th>
      <th colspan="4">B — gemma-4-31b (this run)</th>
...
${rowGrid(reference.cells)}
  ...
  <div class="foot">
    v3 = v2's pipeline with the HEAD PHASES REUSED (script, blueprints, design system all fixed to v2's artifacts;
...
const main = async (): Promise<void> => {
  if (!castConfigured()) {
    console.error("RB_CAST_KEY missing — run: set -a && source .env.local && set +a && node scripts/acceptance3-spike.mjs");
    process.exitCode = 1;
    return;
  }
  ...
  const report: Record<string, unknown> = {
    experiment:
      "ACCEPTANCE V3: the casting decision — v2's Duolingo blueprints/DS/script REUSED VERBATIM (zero head calls); cast executed by zai-glm-4.7 (effort none) and gemma-4-31b (effort omitted) on Cerebras, sequentially; full v2 gate loop per row; 4-row gallery vs gpt-oss (v2) and the GLM-5.2 reference",
    brand: BRAND,
    ref: REF_BUILD,
