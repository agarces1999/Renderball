/**
 * Cycle-10 PRODUCT-PATH validation harness (task #205).
 *
 ...
 *     node scripts/cycle10-product-path.mjs
 */
...
const OUT_DIR = path.join(process.cwd(), ".data", "dogfood", `cycle10-${TAG}`);
...
const main = async (): Promise<void> => {
  console.log(`\n▶ cycle-10 PRODUCT-PATH build — brief ${BRIEF_ID} (${TAG})`);
  ...
  // RB_DOGFOOD_LOCALE=es → a DISCLOSED brief-level instruction that the video copy
  // be in the brand's actual market language (Spanish, for a LATAM retailer like
  // Falabella). The script generator otherwise defaults to English even from a
  // Spanish crawl; this is the faithful output for the brand AND the only way the
  // Spanish/accented-glyph guard is exercised end-to-end (task Part B).
  const localeDirective =
    process.env.RB_DOGFOOD_LOCALE === "es"
      ? `\n\nIDIOMA DEL VIDEO: escribe TODO el texto en pantalla en ESPAÑOL — es el idioma de mercado de la marca (retail LATAM). Usa acentos y signos correctos (á é í ó ú ñ ü ¿ ¡); copy real de e-commerce chileno/latinoamericano.`
      : "";
  const agentBrief: AgentBrief = {
    ...
    freeform_prompt: (freeform || `Launch video for ${brand}.`) + localeDirective,
  };
