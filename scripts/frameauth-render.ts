...
(async () => {
  ...
  const logoSrc: string | undefined = userLogo?.url ?? be?.logo_hd ?? be?.apple_touch_icon ?? be?.favicon ?? undefined;
  ...
  // ── frames: prefer the loop's final measurements (already rendered); else
  //    render once from the shipped composition. ──
  await fs.writeFile(path.join(GEN_DIR, "Composition.tsx"), loop.finalCode, "utf8");
  let frames = loop.finalMeasurements;
  if (!frames.some((m) => m.screenshotPath)) {
    const outMeasure = path.join(OUT, "_measure");
    await fs.mkdir(outMeasure, { recursive: true });
    frames = await measureScenes(GEN_DIR, composedScript, outMeasure);
  }
