...
check("docx: paragraphs come out as separate lines, entities decoded", () => {
  const r = extractText(docx(PARAS), "brief.docx");
...
check("docx: a local header with its own extra length still reads (offset trap)", () => {
  const r = extractText(docx(PARAS, 11), "brief.docx");
...
check("docx detected by BYTES even when the extension lies", () => {
  const r = extractText(docx(PARAS), "brief.txt");
  assert(r.ok && r.text.includes("Fuse Finance"), `sniffing failed: ${r.reason ?? r.text.slice(0, 60)}`);
});

// ── pdf ─────────────────────────────────────────────────────────────────────
check("pdf is refused with a route forward, not a dead end", () => {
  const r = extractText(Buffer.from("%PDF-1.7\n%����\n1 0 obj"), "brief.pdf");
  assert(!r.ok, "a PDF must not be scraped into mojibake");
  assert(/paste/i.test(r.reason ?? ""), `the message must say what to do instead: ${r.reason}`);
});

check("a PDF renamed .txt is still caught by its bytes", () => {
  const r = extractText(Buffer.from("%PDF-1.4\nbinary"), "brief.txt");
  assert(!r.ok && /PDF/i.test(r.reason ?? ""), `expected the PDF message, got: ${r.reason}`);
});

// ── plain text ──────────────────────────────────────────────────────────────
check("plain text passes through, whitespace tidied", () => {
  const r = extractText(Buffer.from("Line one.   \r\n\r\n\r\n\r\nLine two.\t\tTabbed."), "notes.md");
  assert(r.ok, `expected ok: ${r.reason}`);
  assert(r.text === "Line one.\n\nLine two. Tabbed.", `unexpected tidy: ${JSON.stringify(r.text)}`);
});

check("binary junk named .txt is refused rather than poisoning the brief", () => {
  const junk = Buffer.alloc(400);
  for (let i = 0; i < junk.length; i++) junk[i] = 0x80 + (i % 60);
  const r = extractText(junk, "brief.txt");
  assert(!r.ok, "binary must not become brief text");
});

check("an unsupported extension names itself and offers the way through", () => {
  const r = extractText(Buffer.from("x"), "deck.pptx");
...
check("an empty file is refused", () => {
  const r = extractText(Buffer.from("   \n  "), "empty.txt");
  assert(!r.ok, "empty must not count as attached");
});

// ── size ────────────────────────────────────────────────────────────────────
check("oversized input is clamped and flagged, not rejected", () => {
  const big = Buffer.from(("Paragraph about the close.\n\n".repeat(4000)));
  const r = extractText(big, "long.txt");
