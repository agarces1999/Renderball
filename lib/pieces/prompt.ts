...
export const PIECE_SPEC_PROMPT = [
  ...
  `It compiles to brand-tokened markup at build time. Pick the variant that fits THIS scene's register — do not default to the same variant every time; vary variants across the deck's scenes — but each FACT appears exactly once: never render the same value/label in two variants or two markers. The value MUST be copied verbatim from this scene's copy slots (headline, meta values, bullets — e.g. if the headline is the number, value = that exact string); never write a filler like PLACEHOLDER — if no real number exists in the slots, do not emit a statTile at all. Everything richer — charts, diagrams, UI mocks, timelines, any composition with a point of view — you hand-write as always.`,
].join("\n");
