...
// FAST router since 2026-07-23 ($2.10/$6.60, $0.21 cached input). These must
// NOT fall through to the Sonnet fallback (the bug this locks against):
// MODELS.* build/QA stages bill on glm-5.2.
check("GLM-5.2 base rate: 1M in = $2.1, 1M out = $6.6 (fast router, not Sonnet fallback)", () => {
...
check("GLM cache READ bills at Fireworks' real cached-input price ($0.21/M), not the Anthropic 10%", () => {
...
check("GLM cache WRITE bills at plain input rate (no documented Fireworks premium)", () => {
