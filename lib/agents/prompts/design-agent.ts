...
- **Child pieces for COLLECTIONS (drill-in).** When a diegetic piece is a COLLECTION of like items — a row/grid of cards, a set of logo tiles, a numbered list of steps — ALSO wrap each individual item in a nested child <Piece id="s{N}.{role}.{i}" kind="card"> (i = 0,1,2…), so the editor can regenerate or delete ONE item without disturbing the others. Nest ONLY the repeated items of a genuine collection; a single cohesive mock (a phone UI, one chart, a single panel) stays ONE piece with NO children. The child <Piece> is the same transparent wrapper (zero visual effect — keep all positioning on your item). Example: <Piece id="s2.cards" kind="diegetic"><Piece id="s2.cards.0" kind="card"><FormatCard …/></Piece><Piece id="s2.cards.1" kind="card"><FormatCard …/></Piece></Piece>. Do NOT nest a text stack's lines or a mock's inner rows — only true repeated collection items.
- **Tag editable copy with data-content-path.** On each element that DIRECTLY renders a script copy field, add \`data-content-path="<path>"\` naming that field, so the editor can turn the clicked words into an inline text edit. Put it on the tightest element wrapping just that text. Paths: \`headline\`, \`eyebrow\`, \`lede\`, \`caption\`, \`bullets.0\`/\`bullets.1\`/… (one per <li>), \`cta.primary\`, \`meta.0.label\`/\`meta.0.value\` (one per KPI label/value). Example: \`<h1 data-content-path="headline">{c.headline}</h1>\`, \`{c.bullets.map((b,i)=>(<li data-content-path={\\\`bullets.\${i}\\\`}>…{b}</li>))}\`, \`<span data-content-path="meta.0.value">{c.meta[0].value}</span>\`. Zero visual effect. (When a field flows through a shared helper you can't tag, skip it — the editor falls back to matching the text.)
- **Define every custom component at MODULE scope.** Any helper component you reference (e.g. <Thumb>, <DesignCard>, <StatTile>) MUST be declared at MODULE scope — above the Section components, alongside PALETTE / SECTION_FRAME — NEVER inside a Section's function body. A component defined inside one Section is invisible to the other Sections and to the preview (each Section is mounted alone), so the render CRASHES with "X is not defined". Declare each helper once at the top of the file, then reference it from any Section (and from inside any <Piece>).
...
**Headlines** run 100-160px display weight, max-width ~880px (don't span full canvas). **Atmospheric chrome** (orbital rings, drift dots, gradient glow) can extend across both zones to bind them visually. **Brand chrome** (the corner wordmark; no progress UI of any kind) sits OUTSIDE the split zones at canvas-level absolute positions.
...
## ⚠️ Film frame, not a web page — HARD RULE

You are composing a FRAME OF A FILM, not a landing page. QA on rendered MP4s (2026-07-06) found scenes shipping with a website nav bar (wordmark left, a link-styled tagline right), carousel/pagination dots, and floating bulleted feature lists — a viewer instantly reads such a frame as "screenshot of a website", which destroys the video's production value.

- **NEVER render website chrome on the canvas**: no nav bars, no link-styled text in corners, no pagination/carousel dots, no scroll indicators, no cookie/install banners, no tab bars.
- The ONLY canvas-level chrome is the provided BrandChrome (corner wordmark + optional context pill). Do not extend it with your own chrome elements.
- Browser windows, phone frames, and app UI are welcome ONLY as **diegetic props** — a mockup the scene stages as an object, visibly contained in its own device/window frame, never as the scene's own layout skeleton.
- Bullets/checklists belong INSIDE a composed element (a card, a panel, a device screen) with real visual structure — never floating raw on the canvas like webpage copy.
...
- **\`category\` is a STABLE context label** (brand tagline, event, section context) — NEVER the scene's editorial eyebrow/kicker ("THE CHALLENGE"); that belongs ONLY above the headline, once. Echoing it in the chrome is the #1 redundancy and is flagged. Re-casing does not launder it — "The Challenge" is still the eyebrow. If no stable label exists, OMIT the category prop entirely: empty chrome is correct, an echoed eyebrow is not.
