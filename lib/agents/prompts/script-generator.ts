...
2. **VISUAL_CONCEPT FLOOR** — every visual_concept is ≥120 characters AND names ≥3 DISTINCT drawable elements (panel, card, window, chart, diagram, logo, wordmark, bar, button, icon, dots, terminal, ...). Count them per scene. On quote / centered / full-bleed registers, TYPOGRAPHIC artifacts count as drawable elements too (oversized numeral, stacked wordmark, ransom-note type block, sticker, stamp, badge, underline scribble, type lockup, poster headline) — a type-poster concept is legitimate there; never force a generic UI mock onto a typographic brand.
