...
3. **FURNISHED INTERIORS** — any visual_concept naming a container (${containerVocabulary().join(" / ")}) must ALSO name ≥2 specifics drawn INSIDE it. **THIS IS A LITERAL WORD-MATCH AGAINST A CLOSED LIST, exactly like item 2.** Only these count: ${interiorVocabulary().join(", ")}. "Three window mockups" with nothing named inside FAILS — and so does "approval cards showing a name, an amount and a status pill", which counts ZERO, because name, amount and pill are not on the list. Write "rows with labels and values" and it counts two. If you cannot furnish a container, do not name one: describe what is actually there using item 2's non-container words instead.
