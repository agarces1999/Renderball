...
export const buildCompositionPrompt = (
  ...
  const system = [
    ...
    ``,
    `FULL-BLEED SHAPE (author LEAN, not maximal): on a "full-bleed" register scene the hero IS the frame. Build it as ONE primary surface (a single dashboard / window / app shell) plus a FLAT list of interior rows or tiles — a TEMPLATE shape, never a deeply-nested tree. Keep the full-bleed hero's interior inventory to 6–8 items TOTAL. Anchor a FEW toward the TOP (a nav/header band) and a FEW toward the BOTTOM (a footer/meta strip) so the composition spans the frame without a big empty middle — but do NOT over-stuff or deep-nest it. A maximal, deeply-nested full-bleed hero exceeds the emit budget and TRUNCATES before it closes (shipping a broken/fallback panel); a few well-placed anchoring items top and bottom, flatly nested, read as a full frame AND emit cleanly.`,
    `FULL-BLEED SINGLE SURFACE + LEGIBLE COPY: a full-bleed scene shows exactly ONE primary product surface — one dashboard, one window, one app shell. Do NOT stack a second full mock (a chat/assistant modal layered behind the first reads as a rendering artifact), and never float an EMPTY container over the surface (a solid card carrying no text/mock reads as a leftover box — a blocking defect). When the scene ALSO casts a standalone copy element, give the copy its OWN solid-backed panel in a corner band (e.g. a lower-left card) whose bounds sit in the surface's QUIET margin, NOT over its dense interior (charts, rows, tiles) — copy painted over a busy region is buried and unreadable. Reserve that copy band clear of the mock's data-dense zone.`,
    ``,
    `SPLIT / LIST SHAPE (carry BOTH columns): on a "split" (or a two-column "list") scene the frame is TWO columns and BOTH must carry substantial content — the mock/hero column on ONE side and the copy column (or a second panel / rows / tiles) on the OTHER, on DIFFERENT sides. Author the hero's bounds to own one half (e.g. x 60–900) and the copy's bounds to own the OPPOSITE half (e.g. x 1020–1820). NEVER stack BOTH the mock AND the copy on the same side and leave the other half empty — an authored-empty left or right half is the "abandoned half" defect: it ships as a dead white/canvas column and is a BLOCKING failure the head must fix. If one column would be thin, give it a real secondary element (a supporting stat panel, a caption/annotation strip, a second small mock) so neither half reads abandoned.`,
