...
export const FIT_TEXT_SCRIPT: string = [
  ...
  "  } else { start(); }",
  "  /* Re-entry for MORPHED documents (speed playbook): a DOM-patched update",
  "     replaces changed nodes' style attributes, wiping their previous fit —",
  "     the parent editor calls this after morphing instead of reloading. The",
  "     done-promise resets so measure-style waiters stay correct. */",
  "  window.__rbRefit = function () {",
  "    window.__rbFitDone = new Promise(function (r) { resolveDone = r; });",
  "    window.__rbFitSummary = { fitted: 0, floored: 0, candidates: 0 };",
  "    run();",
  "  };",
