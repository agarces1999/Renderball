...
import { mergeProvenance, recordProvenance, readProvenance, type ProvenanceMap } from "./provenance";
...
await check("mergeProvenance keeps a cached descriptor through a route re-record", async () => {
  ...
  try {
    // Generation day: route records origin+prompt, then merges genMeta.
    await recordProvenance(dir, "s0.add1", { origin: "marquee", prompt: "a rocket" });
    ...
    // A later regen re-records origin+prompt (recordProvenance REPLACES)…
    await recordProvenance(dir, "s0.add1", { origin: "regen", prompt: "make it warmer" });
    ...
    const e = map["s0.add1"];
    assert(e.origin === "regen" && e.prompt === "make it warmer", "record wins on origin/prompt");
